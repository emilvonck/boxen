...
const (
	FortinetFortigateDefaultUser = "admin"
	FortinetFortigateDefaultPass = "admin"

	FortinetFortigateScrapliPlatform = "cisco_iosxe" // nolint:lll

	FortinetFortigateDefaultBootTime = 720
)
...
func (p *FortinetFortigate) Package(
	_, _ string,
) (packageFiles, runFiles []string, err error) {
	return nil, nil, err
}

func (p *FortinetFortigate) Install(opts ...instance.Option) error {
	...
	go func() { //nolint:dupl
		...
		err = p.login(
			&loginArgs{
				username: FortinetFortigateDefaultUser,
				password: FortinetFortigateDefaultPass,
			},
		)
		if err != nil {
			c <- err
...
func (p *FortinetFortigate) Start(opts ...instance.Option) error { // nolint:dupl
...
func (p *FortinetFortigate) startReady() error {
	...
	err = p.readUntil(
		...
		getPlatformBootTimeout(PlatformTypeFortinetFortigate),
	)

	return err
}

func (p *FortinetFortigate) SaveConfig() error {
	p.Loggers.Base.Info("save config requested")

	_, err := p.c.SendCommand(
		"save config",
		sopoptions.WithTimeoutOps(
			time.Duration(getPlatformSaveTimeout(PlatformTypeFortinetFortigate))*time.Second,
		),
	)
...
func (p *FortinetFortigate) SetUserPass(usr, pwd string) error {
	if usr == FortinetFortigateDefaultPass && pwd == FortinetFortigateDefaultPass {
...
func (p *FortinetFortigate) SetHostname(h string) error {
